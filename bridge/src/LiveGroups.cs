 ...
 static object Ungroup(JsonElement req,object svc,object client,object editor){
  ...
  try{
   ...
   foreach(var row in rows){
    var parent=Call(row.group,"JRiO");
    var delete=T("qgLq.mgL7").GetMethod("mgB8",F).Invoke(null,new object[]{Typed("wRsm.aRsa",new[]{row.group}),retain});
    edits.Add(Make("WgDd.VgDq",diagram,Typed("wRsm.aRsa",row.children),row.group,parent,delete,true,true));
   }
   Commit(client,editor,Combined(edits));
   foreach(var row in rows)foreach(var child in row.children){var parent=Call(child,"JRiO");if(parent!=null&&groupIds.Contains(Id(parent)))throw new Exception("Ungroup verification failed");}
