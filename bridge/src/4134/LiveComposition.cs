...
public static partial class LiveOperations {
 ...
 static object TextStyle(object w,string name){var font=Call(Get(w,"AttributedString"),"YKvT");var value=T("Ot7G.nt79").GetMethod("ValueAsObject",F).Invoke(font,new[]{Enum.Parse(T("Axure.Constants.PName"),name)});if(value==null&&name=="FONT_NAME")value=Get(Get(w,"MyStyle"),"FontName");if(value==null&&name=="BOLD")value=Get(Get(w,"MyStyle"),"Bold");return value;}
 static Guid Id(object w)=>(Guid)T("itcK.ftcZ").GetMethod("Rtc0",F).Invoke(w,null);
 ...
 static object Fill(string hex){var raw=Convert.FromHexString(hex.Substring(1));var color=T("qFDd.cFD0").GetMethod("mFDO",F).Invoke(null,new object[]{raw.Length==4?(int)raw[3]:255,(int)raw[0],(int)raw[1],(int)raw[2]});return Make("bjAf.TjAr",color);}
 ...
 static List<object> Patch(object w,JsonElement p,bool capture){
  ...
  if(p.TryGetProperty("text",out var txt)){
   if(capture){var attr=Get(w,"AttributedString");var font=Call(attr,"YKvT");var next=attr.GetType().GetMethod("rsEI",F).Invoke(null,new object[]{txt.GetString(),font});edits.Add(Make("tiyJ.kiyT",w,next));}
   else Set(w,"Text",txt.GetString());
  }
  if(p.EnumerateObject().Any(x=>new[]{"x","y","width","height"}.Contains(x.Name))){
   if(capture)edits.Add(Make("zs16.os1u",w));
   ...
   Call(w,"Move",Make("yFJ2.mFJS",x-(double)Get(r,"X"),y-(double)Get(r,"Y")),Widgets(w),false,false,false,null);
   Set(style,"Size",Make("sjHY.YjHa",width,height));
  }
  var visual=p.EnumerateObject().Where(x=>!new[]{"text","x","y","width","height"}.Contains(x.Name)).ToArray();
  if(visual.Length>0){
   if(capture)edits.Add(Make("MsV3.qsVc",Widgets(w),false));
   ...
   foreach(var v in visual){
    switch(v.Name){
     ...
     case "imageBase64":var stream=new MemoryStream(DecodePng(v.Value.GetString()),false);Set(w,"Image",Make("TtOw.ytOe",stream,"live-image.png",Guid.NewGuid().ToString(),false));break;
    }
   }
   if(fontFields.Count>0){var props=((IEnumerable)Get(fontStyle,"Props")).Cast<object>().Where(v=>fontFields.Contains(Call(v,"aFIe").ToString()));Call(w,"LKNO",Typed("CFIK.CFIZ",props),false);}
  }
  return edits;
 }
 static void Commit(object client,object editor,object edit){Call(editor,"MK8e",edit,false);if(!ReferenceEquals(edit,NativeTop(client,editor,false)))throw new Exception("Native undo registration failed");UndoOwned.Push(edit);RedoOwned.Clear();}
 static object Combined(IEnumerable<object> edits)=>Make("DsjR.RsjO",new object[]{Typed("ssKX.HsKR",edits)});
 ...
 static object Composition(JsonElement req,object svc,object client,object editor){
  ...
  try{
   if(action=="compose"){
    ...
    var edit=Make("MsZ2.ysZS",Enum.Parse(T("Axure.DocumentModel.Edit.DiagramEditAction"),"Add"),diagram,Typed("mGXa.OGXN",new[]{root}.Concat(created)));
