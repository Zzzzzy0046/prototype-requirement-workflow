...
public static partial class LiveOperations {
 ...
 static object TextStyle(object w,string name){var font=Call(Get(w,"AttributedString"),"la5q");var value=T("SoDR.MoDL").GetMethod("ValueAsObject",F).Invoke(font,new[]{Enum.Parse(T("Axure.Constants.PName"),name)});if(value==null&&name=="FONT_NAME")value=Get(Get(w,"MyStyle"),"FontName");if(value==null&&name=="BOLD")value=Get(Get(w,"MyStyle"),"Bold");return value;}
 static Guid Id(object w)=>(Guid)T("WoYu.hoYa").GetMethod("aoYw",F).Invoke(w,null);
 ...
 static List<object> Patch(object w,JsonElement p,bool capture){
  ...
  if(p.TryGetProperty("text",out var txt)){
   if(capture){var attr=Get(w,"AttributedString");var font=Call(attr,"la5q");var next=attr.GetType().GetMethod("gEp8",F).Invoke(null,new object[]{txt.GetString(),font});edits.Add(Make("H8JD.w8Jq",w,next));}
   else Set(w,"Text",txt.GetString());
  }
  if(p.EnumerateObject().Any(x=>new[]{"x","y","width","height"}.Contains(x.Name))){
   if(capture)edits.Add(Make("hEOQ.rEO9",w));
   ...
   Call(w,"Move",Make("oGqb.eGqf",x-(double)Get(r,"X"),y-(double)Get(r,"Y")),Widgets(w),false,false,false,null);
   Set(style,"Size",Make("VyK5.myK1",width,height));
  }
  var visual=p.EnumerateObject().Where(x=>!new[]{"text","x","y","width","height"}.Contains(x.Name)).ToArray();
  if(visual.Length>0){
   if(capture)edits.Add(Make("zE6I.pE6l",Widgets(w),false));
   ...
   foreach(var v in visual){
    switch(v.Name){
     ...
     case "imageBase64":var stream=new MemoryStream(DecodePng(v.Value.GetString()),false);Set(w,"Image",Make("LoQJ.yoQs",stream,"live-image.png",Guid.NewGuid().ToString(),false));break;
    }
   }
   if(fontFields.Count>0){var props=((IEnumerable)Get(fontStyle,"Props")).Cast<object>().Where(v=>fontFields.Contains(Call(v,"xG7s").ToString()));Call(w,"AaUx",Typed("JG7u.XG7a",props),false);}
  }
  return edits;
 }
 static void Commit(object client,object editor,object edit){Call(editor,"laSs",edit,false);if(!ReferenceEquals(edit,NativeTop(client,editor,false)))throw new Exception("Native undo registration failed");UndoOwned.Push(edit);RedoOwned.Clear();}
 static object Combined(IEnumerable<object> edits)=>Make("tEye.tEyx",new object[]{Typed("oEap.gEae",edits)});
 ...
 static object Composition(JsonElement req,object svc,object client,object editor){
  ...
  if(!ReferenceEquals(fragment,Call(client,"XapS")))throw new Exception("Target page must be open");
  ...
  try{
   if(action=="compose"){
    ...
    var edit=Make("MEWb.vEWf",Enum.Parse(T("Axure.DocumentModel.Edit.DiagramEditAction"),"Add"),diagram,Typed("aLe1.CLe0",new[]{root}.Concat(created)));
