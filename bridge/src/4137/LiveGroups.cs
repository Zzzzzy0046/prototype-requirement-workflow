 ...
 static object Ungroup(JsonElement req,object svc,object client,object editor){
  ...
  try{
   ...
   foreach(var row in rows){
    var parent=Call(row.group,"lLhU");
    var delete=T("MEWb.vEWf").GetMethod("oEWg",F).Invoke(null,new object[]{Typed("aLe1.CLe0",new[]{row.group}),retain});
    edits.Add(Make("iEgH.OEgb",diagram,Typed("aLe1.CLe0",row.children),row.group,parent,delete,true,true));
   }
   Commit(client,editor,Combined(edits));
   foreach(var row in rows)foreach(var child in row.children){var parent=Call(child,"lLhU");if(parent!=null&&groupIds.Contains(Id(parent)))throw new Exception("Ungroup verification failed");}
