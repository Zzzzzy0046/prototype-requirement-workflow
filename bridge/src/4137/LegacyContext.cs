...
internal static class LegacyNative {
 ...
 internal static Guid Id(object o)=>(Guid)App.GetType("WoYu.hoYa",true).GetMethod("aoYw",F).Invoke(o,null);
 internal static Guid PageId(object o)=>(Guid)Call(o,"tock");
...
internal sealed class LegacyDirectory {
 public static object[] OpenClients(){
  var t=LegacyNative.App.GetType("cut.euS",true);var result=new List<object>();
...
internal sealed class LegacyDocumentService {
 ...
 public object LoadFragment(Guid id){var current=LegacyNative.Call(client,"XapS");if(current==null||LegacyNative.PageId(current)!=id){var pages=(IEnumerable<PageNode>)((Sitemap)GetSitemap("active")).Pages;if(!Flatten(pages).Any(p=>p.Id==id.ToString()&&p.Type=="Page"))throw new Exception("Unknown page ID");LegacyNative.Call(client,"Gapc",id,Guid.Empty,false);current=LegacyNative.Call(client,"XapS");}if(current==null||LegacyNative.PageId(current)!=id)throw new Exception("Page activation failed");return current;}
 ...
 public object GetSitemap(string active){
  var project=LegacyNative.Call(client,"Wap4");var tree=LegacyNative.Call(project,"CLQF");var root=LegacyNative.Call(tree,"cRvX");
  List<PageNode> Visit(object parent){var result=new List<PageNode>();foreach(var node in (IEnumerable)LegacyNative.Call(parent,"fLdd")){var val=LegacyNative.Call(tree,"oRv4",node);Guid id=val is Guid g?g:(Guid)LegacyNative.Call(val,"yLdH");result.Add(new PageNode{Id=id.ToString(),Name=(string)LegacyNative.Call(tree,"va3X",node),Type="Page",Children=Visit(node)});}return result;}
  return new Sitemap{FileName=Path.GetFileName(LegacyNative.Document(client)),Pages=Visit(root)};
 }
 public static object ResolveDiagram(object fragment,Guid? sub){if(sub!=null)throw new Exception("Legacy subdiagram access not supported");return LegacyNative.Call(fragment,"gL5k");}
 ...
 public object GetScreenshot(string active,Guid id,Guid? sub,IReadOnlyList<Guid> ids,int max){
  ...
  if(ids==null||ids.Count==0)bitmap=LegacyNative.Call(diagram,"xLxB",false,false,false,false);
  else{var array=Array.CreateInstance(LegacyNative.App.GetType("aLe1.CLe0",true),ids.Count);for(int i=0;i<ids.Count;i++)array.SetValue(ResolveWidget(fragment,ids[i].ToString(),null),i);bitmap=LegacyNative.Call(diagram,"FLxv",false,array,null,false);}
  try{using var stream=(Stream)LegacyNative.Call(bitmap,"euBe",Enum.Parse(LegacyNative.App.GetType("Axure.Platform.AxImageFormat",true),"Png",true),false);using var memory=new MemoryStream();stream.CopyTo(memory);return new{Data=Convert.ToBase64String(memory.ToArray()),MimeType="image/png",Description="Axure native page model render (legacy adapter)"};}finally{(bitmap as IDisposable)?.Dispose();}
 }
}
