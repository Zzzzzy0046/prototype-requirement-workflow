...
public static partial class LiveOperations {
 static object CreateShapes(JsonElement req,object svc,object client,object editor){
  ...
  var name=req.GetProperty("groupName").GetString();if(string.IsNullOrWhiteSpace(name)||name.Length>100)throw new Exception("Invalid group name");
  ...
  try{
   var root=Make("Axure.DocumentModel.Objects.Layer",fragment);Set(root,"Name",name);
   for(int i=0;i<leaves.Count;i++){Patch(leaves[i],items[i].GetProperty("patch"),false);InvokeExact(root,"Add",leaves[i],i);}
   var diagram=svc.GetType().GetMethod("ResolveDiagram",F).Invoke(null,new object[]{fragment,null});
   Commit(client,editor,Make("qgLq.mgL7",Enum.Parse(T("Axure.DocumentModel.Edit.DiagramEditAction"),"Add"),diagram,Typed("wRsm.aRsa",new[]{root}.Concat(leaves))));
   var result=new{groupId=Id(root),count=leaves.Count,widgets=leaves.Select(w=>new{widgetId=Id(w),state=State(w),fingerprint=Fingerprint(State(w))}).ToArray(),saved=false};
