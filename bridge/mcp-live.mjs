...
tool('axure_live_create_shapes','Create independent editable native Rectangle/Paragraph/Ellipse leaves on a blank or existing current page in one native undo operation. No Axure Group is created.',{...batch,items:z.array(z.object({shape:z.enum(['Rectangle','Paragraph','Ellipse']),name:z.string().min(1).max(100),patch}).strict()).min(1).max(150)},'create-shapes');
...
tool('axure_live_compose','Clone 1-150 existing native leaf templates, preserve source resources, apply patches, and add independent leaves to the CURRENT page as ONE native undo operation. No Axure Group or clipboard. Input order is back-to-front. Copies have independent IDs. PNG import supported; no arbitrary files/URLs. Unchanged text preserves full source rich text.',{...batch,items:z.array(z.object({templateId:z.string().uuid(),expectedFingerprint:fingerprint,name:z.string().min(1).max(100),patch}).strict()).min(1).max(150)},'compose');
...
tool('axure_live_ungroup','Remove bridge-created Axure Groups while preserving mapped leaf IDs, geometry, style, order, and native undo. Refuses mixed, nested, or unexpected group membership.',{...batch,groupIds:z.array(z.string().uuid()).min(1).max(50),expectedWidgetIds:z.array(z.string().uuid()).min(1).max(500)},'ungroup');
