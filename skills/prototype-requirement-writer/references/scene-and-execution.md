...
`bridge/planning.mjs` 校验需求、必要问答及表达映射；`bridge/scene.mjs` 校验场景。先读 page-expression.md，参考 `examples/clarification-home/` 完整问答正例。旧 `examples/printer/` 仅为技术兼容示例。Agent 负责理解需求，脚本不是 AI 模型，不会从一句话自动推断产品规则。
...
```json
{
  ...
  "pages": [{
    ...
    "widgets": [{
      "key": "title", "shape": "Paragraph", "text": "My App",
      "x": 24, "y": 28, "width": 300, "height": 40,
      "fontSize": 18, "bold": true
    }],
    "notes": [{"key": "purpose", "title": "页面说明", "lines": ["1、说明此页面解决什么问题。"]}]
  }]
}
```
...
当前 runner 新建的是普通页面，不支持自动创建页面树文件夹。使用与本 skill 匹配的 1.7.0 包，不静默丢弃字段或调用旧版 runner。新 scene 使用 noteProfile=house-review；default 和 android-review 仅用于既有项目兼容，未指定仍保持旧输出。通用几何、说明字体与宽度见 house-review-profile.md；HTML 与 Axure 使用相同编译结果，真实字形/行距仍需核对。notes.kind 和 table 见 note-expression.md；不需要修改底层适配器，但本版未重新完成三版本原生测试。
...
node .\bridge\workflow.mjs apply --spec 'C:\AxureTrials\Project\scene.json' --out 'C:\AxureTrials\Project\output' --session '.\bridge\sessions\trial\session.json' --project 'C:\AxureTrials\Project\Prototype.rp'
```
...
- `axure-render/*.png`：Axure 原生整页渲染，不依赖临时分组是否仍存在。
